...
// Replace with your actual screens / paths:
import 'add_organization_screen.dart'; // screen to add/edit an org
...
class _MarketplaceAllScreenState extends State<MarketplaceAllScreen>
  ...
  Future<void> _fetchUserRole() async {
    ...
    if (mounted) setState(() => _isAdmin = data != null && data['role'] == 'admin');
  }
    ...
    await showModalBottomSheet(
      ...
      builder: (ctx) {
        return StatefulBuilder(builder: (ctx2, setModal) {
          final subitems = tmpCat.isEmpty ? <String>[] : (_catMap[tmpCat] ?? <String>[]);
          return Padding(
            padding: EdgeInsets.only(left: 16, right: 16, top: 12, bottom: MediaQuery.of(ctx2).viewInsets.bottom + 12),
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: Text('Filters', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.primary))),
                    IconButton(
                      onPressed: () {
                        setModal(() {
                          tmpCat = '';
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: ['','Healthcare','Education','Retail','Food & Dining','Services','Other']
                      .where((e)=> e!=null)
                      .map((c) => DropdownMenuItem(value: c, child: Text(c.isEmpty ? 'Any' : c))).toList(),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: (tmpCat.isEmpty ? <String>[] : (_catMap[tmpCat] ?? [])).map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
  ...
  Future<void> _deleteDoc(String id) async {
    try {
      await FirebaseFirestore.instance.collection(kOrganizationsCollection).doc(id).delete();
  ...
  Widget _imgFallback() => const Center(child: Icon(Icons.storefront_outlined, color: Colors.white54));
  ...
  Future<void> _launchWebsite(String url) async {
    if (url.isEmpty) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No website provided')));
      return;
    }
    ...
    if (await canLaunchUrl(uri)) {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cannot open website')));
    }
  }
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ClipRRect(
              ...
              child: imageUrl.isNotEmpty
                  ? Image.network(imageUrl, height: 180, width: double.infinity, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _imgFallback())
                  : Container(height: 180, width: double.infinity, color: Colors.white10, child: _imgFallback()),
            ),
            const SizedBox(height: 12),
            Align(alignment: Alignment.centerLeft, child: Text(name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800))),
            const SizedBox(height: 6),
            Align(alignment: Alignment.centerLeft, child: Text('$category${subcategory.isNotEmpty ? ' • $subcategory' : ''}', style: const TextStyle(fontSize: 14, color: Colors.black54))),
            const SizedBox(height: 12),
            Text(desc, style: const TextStyle(fontSize: 14, height: 1.4)),
            const SizedBox(height: 12),
            if (address.isNotEmpty) ...[
              Row(children: [const Icon(Icons.location_on_outlined, size: 18, color: Colors.grey), const SizedBox(width: 8), Expanded(child: Text(address, style: const TextStyle(color: Colors.black54)))]),
              const SizedBox(height: 8),
            ],
            Row(children: [
              Expanded(child: ElevatedButton.icon(icon: const Icon(Icons.call), label: const Text('Call'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary), onPressed: phone.isNotEmpty ? () => _launchTel(phone) : null)),
              const SizedBox(width: 8),
              Expanded(child: OutlinedButton.icon(icon: const Icon(Icons.chat_bubble_outline), label: const Text('Chat'), onPressed: () {
            ...
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: OutlinedButton.icon(icon: const Icon(Icons.language), label: const Text('Website'), onPressed: () => _launchWebsite(website))),
              const SizedBox(width: 8),
              Expanded(child: OutlinedButton.icon(icon: const Icon(Icons.map_outlined), label: const Text('Directions'), onPressed: () => _openMaps(address))),
            ]),
            if (canManage) ...[
              const SizedBox(height: 12),
              Row(children: [
                Expanded(
                  child: OutlinedButton(
                    onPressed: () {
                      Navigator.pop(ctx);
                      Navigator.push(context, MaterialPageRoute(builder: (_) => AddOrganizationScreen(existingData: data, docId: docId)));
                    },
                    ...
                    onPressed: () async {
                      final confirm = await showDialog<bool>(
                        ...
                        builder: (_) => AlertDialog(
                          ...
                          content: const Text('Do you want to delete this organisation?'),
                          actions: [
      ...
      child: Container(
        ...
        child: Row(children: [
          ClipRRect(
            ...
            child: imageUrl.isNotEmpty ? Image.network(imageUrl, width: 92, height: 92, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _imgFallback()) : Container(width: 92, height: 92, color: Colors.grey[100], child: _imgFallback()),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 6),
              Text('$category${sub.isNotEmpty ? ' • $sub' : ''}', style: const TextStyle(fontSize: 13, color: Colors.black54)),
              const SizedBox(height: 6),
              Text(description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 14)),
              const SizedBox(height: 8),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                if (phone.isNotEmpty) Text(phone, style: const TextStyle(fontSize: 13, color: Colors.grey)),
                if (canManage)
                  Row(children: [
                    IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AddOrganizationScreen(existingData: data, docId: doc.id))), icon: const Icon(Icons.edit)),
                    IconButton(onPressed: () async {
                      final confirm = await showDialog<bool>(
                        context: context,
                        builder: (_) => AlertDialog(
                          title: const Text('Delete'),
                          content: const Text('Delete this organisation?'),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ClipRRect(
            ...
            child: imageUrl.isNotEmpty ? Image.network(imageUrl, height: 120, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _imgFallback()) : Container(height: 120, color: Colors.grey[100], child: _imgFallback()),
          ),
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 6),
              Text('$cat${sub.isNotEmpty ? ' • $sub' : ''}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
            ]),
          ),
        ]),
      ),
  ...
  Widget _buildTabStream({required bool onlyMine}) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        final docs = snap.data!.docs.where((d) => _matchesSearch(d.data() as Map<String, dynamic>)).toList();
        if (docs.isEmpty) return const Center(child: Text('No organisations found'));
                      ...
                      if (_subcategoryFilter.isNotEmpty)
                        Padding(
                          ...
                          child: InputChip(
                            ...
                            onDeleted: () => setState(() => _subcategoryFilter = ''),
                          ),
                        ),
                      Padding(
                        padding: const EdgeInsets.only(right: 8),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [Tab(text: 'All MarketPlace'), Tab(text: 'My MarketPlace')],
        ),
        actions: [
      ...
      body: Column(children: [
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(hintText: 'Search name, category, subcategory, phone...', border: InputBorder.none, isDense: true),
                    ),
                  ),
                  if (_searchCtrl.text.isNotEmpty)
                    IconButton(icon: const Icon(Icons.clear_rounded, size: 20), onPressed: () => setState(() {
                      _searchCtrl.clear();
                      _search = '';
                    })),
                ]),
                ...
                child: ChoiceChip(
                  ...
                  onSelected: (_) => setState(() {
                    if (selected) {
                      ...
                      _categoryFilter = c;
                      _subcategoryFilter = ''; // clear sub when category changes
                    }
                  }),
        ...
        // Tabs content
        Expanded(
          child: TabBarView(controller: _tabController, children: [_buildTabStream(onlyMine: false), _buildTabStream(onlyMine: true)]),
        ),
      ]),
